...
//	P2PAPI_Connect(m_hP2PHandle, "VSTC172299NEWGD", "admin", "123456",1);
	...
	long nRet = P2PAPI_Connect(m_hP2PHandle, "VSTG524575XBEEE", "admin", "20190924",1);
...
void CTestP2PDlg::MessageCallBack(long nHandle, int type, char *msg, int len, void *pParam)
{
    ...
    switch (type)
    {
    case MSG_TYPE_P2P_STATUS:  //P2P连接状态
        {
...
void CTestP2PDlg::OnBnClickedBtnStopVideo()
{
	...
	int nCmdU = CMD_PTZ_UP_STOP;
	int nCmdD = CMD_PTZ_DOWN_STOP;
	int nCmdL = CMD_PTZ_LEFT_STOP;
	int nCmdR = CMD_PTZ_RIGHT_STOP;
	P2PAPI_SendMessage(m_hP2PHandle, MSG_TYPE_DECODER_CONTROL, (char*)& nCmdU, sizeof(int));
	P2PAPI_SendMessage(m_hP2PHandle, MSG_TYPE_DECODER_CONTROL, (char*)& nCmdD, sizeof(int));
	P2PAPI_SendMessage(m_hP2PHandle, MSG_TYPE_DECODER_CONTROL, (char*)& nCmdL, sizeof(int));
	P2PAPI_SendMessage(m_hP2PHandle, MSG_TYPE_DECODER_CONTROL, (char*)& nCmdR, sizeof(int));

	long nRet = P2PAPI_StopVideo(m_hP2PHandle);
	DPrintf("\r\n---P2PAPI_StopVideo nRet=%d---", nRet);
}
...
void CTestP2PDlg::OnBnClickedBtnSetUser()
{
    ...
    strcpy(UserInfo.user1, "2385737429@qq.com");
    strcpy(UserInfo.pwd1, "webcam2019");
...
void CTestP2PDlg::OnBnClickedBtnSetFtp()
{
    ...
    strcpy(FtpParams.user, "2385737429@qq.com");
    strcpy(FtpParams.pwd, "webcam2019");
...
void CTestP2PDlg::OnBnClickedBtnSetWifi()
{
    ...
    strcpy(WifiParams.wpa_psk, "dq123456");
...
void CTestP2PDlg::OnBnClickedBtnSetAlarm()
{
	...
	out << "ojbk";
	out.close();
	int AlarmCMD = CMD_PTZ_PREFAB_BIT_RUN0;
	AlarmCMD += b*2;
...
void CTestP2PDlg::OnBnClickedBtnSetPtz()
{
    ...
    ptzParams.disable_preset = 0;
    ptzParams.led_mode = 1;
    ptzParams.ptz_center_onstart = 1;
    ptzParams.ptz_patrol_rate = 4;
    ptzParams.ptz_patrol_up_rate = 4;
    ptzParams.ptz_patrol_down_rate = 4;
    ptzParams.ptz_patrol_left_rate = 4;
    ptzParams.ptz_patrol_right_rate = 4;
	...
	case ID_SETPRESET16:
		{
			...
			int usrCMD = CMD_PTZ_PREFAB_BIT_SET0;
			...
			out.close();
			usrCMD+= b;
			P2PAPI_SendMessage(m_hP2PHandle, MSG_TYPE_DECODER_CONTROL, (char*)& usrCMD, sizeof(int));
			break;
		}
